...
def plot_flow_data(flow_data, time_stamps):
    ...
    flow_data[list(flow_data.keys())[2]].pop()  # Remove the last element of the first flow
    ...
    plt.axhline(y=5, color='green', linestyle='--', label="Requested BW h4-h2 (5M)")
...
# Run it
flows, times = parse_and_label_flows("tmp/h2_server_basic.txt")
