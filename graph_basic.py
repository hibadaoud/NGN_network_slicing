...
# Read data
t_h1_h4 = parse_iperf_file("tmp/h4_basic.txt")
t_h2_h5 = parse_iperf_file("tmp/h5_basic.txt")
t_h3_h6 = parse_iperf_file("tmp/h6_basic.txt")

print("h1-h4 Throughput:", t_h1_h4)
print("h2-h5 Throughput:", t_h2_h5)
print("h3-h6 Throughput:", t_h3_h6)

# remove last element of t_h1_h4
...
plt.plot(x, t_h1_h4, color='g', label="h1 → h4 (7M requested)", marker="o")
plt.plot(x, t_h2_h5, color='b', label="h2 → h5 (2M requested)", marker="s")
plt.plot(x, t_h3_h6, color='orange', label="h3 → h6 (4M requested)", marker="^")

# Plot total throughput
total_throughput = [sum(values) for values in zip(t_h1_h4, t_h2_h5, t_h3_h6)]
...
plt.axhline(y=7, color='g', linestyle='--', label="Requested BW h1-h4 (7M)")
plt.axhline(y=2, color='b', linestyle='--', label="Requested BW h2-h5 (2M)")
plt.axhline(y=4, color='orange', linestyle='--', label="Requested BW h3-h6 (4M)")

plt.title("UDP Throughput Over Time (Congested Link)")
plt.xlabel("Time (s)")
